...
## Extension: Use-Intent Calibration

The calibration above addresses knowledge *quality* dimensions, but a related but distinct gap exists: **calibrating knowledge presentation to the user's *intent* in accessing it**. When someone opens a knowledge base, they have different goals:
...
**Rule (extended):** For mature vaults (>100 notes), explicitly tag primary use intent for each note and create navigation paths optimized for different access intents.
...
**Implementation:** Add `use-intent:` frontmatter to notes and create hub notes per intent type for navigation.

See also: [[Frontier Exploration - Knowledge Calibration by Use Intent]] — this note now integrated.
