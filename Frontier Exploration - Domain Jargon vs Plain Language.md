...
## Proposed Rule (2026-04-01)

> This section integrates insights from gap-identification work on terminology handling.

**Rule:** For notes targeting learning/understanding (vs. reference lookup), include terminology anchors — define domain terms inline or link to terminology notes, while maintaining readability for the primary audience.
**Why:** Knowledge bases serve diverse expertise levels. Notes should be accessible to their primary audience while providing hooks for readers outside that audience.
**Test:** For notes >200 words targeting learning: (1) Does the note define or link all domain-specific terms? (2) If serving novices, is jargon minimized or explained? (3) If serving experts, is terminology precise? (4) Are there terminology lookup notes for cross-domain readers?

### Implementation Options for Terminology

1. **Audience tagging:** `audience-level: beginner|intermediate|expert|universal`
2. **Terminology density:** `jargon-level: minimal|moderate|technical`
3. **Definition linking:** Automatic link generation for recognized domain terms
4. **Plain-language fallback:** Notes can have "Plain Language Summary" sections
5. **Terminology status:** `terminology-status: current|deprecated|emerging|archival`
...
- [[AI-Assisted Knowledge Management Seed]] — Related to terminology (naming guidance)
