...
# Frontier Exploration: Knowledge Freshness

> How should knowledge bases track temporal currency — ensuring knowledge is current rather than just verified? How should the vault actively surface knowledge needing verification?
...
Additionally, the Seed lacks guidance on:
...
| Verification triggers | When does the vault actively surface stale knowledge? |
| Automation hooks | How to integrate cron/heartbeat for automated prompts |
...
### Rule (NEW - from Automation exploration): Implement automated freshness verification triggers based on knowledge type
...
### Rule (NEW): Define resource-constrained verification prioritization

**Why:** In large vaults, verifying all outdated knowledge is impossible. Need explicit prioritization when backlog exceeds capacity.

**Test:** (1) Can you rank stale notes by priority? (2) Does prioritization consider domain velocity and knowledge criticality? (3) Is there a protocol for deferring low-priority verification?

**Implementation:**
```yaml
# Verification priority matrix
verification-priority:
  critical:  # Safety-critical, frequently-used
    frequency: 30d
    queue-weight: high
  high:  # Domain核心 knowledge
    frequency: 90d
    queue-weight: medium-high
  medium:  # Adjacent knowledge
    frequency: 180d
    queue-weight: medium
  low:  # Exploratory/rarely used
    frequency: 365d
    queue-weight: low
```
...
### Active vs. Passive Vaults
Should a knowledge vault actively surface verification needs or wait for humans to query? Recommendation: Active surfacing for critical knowledge, passive for low-priority.

### Multi-Person Vaults
Who gets verification requests? Round-robin by domain expertise, or expertise-based assignment.
...
The Seed also needs automation guidance — when and how the vault should actively surface knowledge needing verification, not just wait for human queries.

Adding freshness guidance would:
...
- [[Seed Refinement - Verification Priority Adaptations]]
- [[_root]]