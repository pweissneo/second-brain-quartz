...
Beyond the core Seed rules, these implementation options help operationalize terminology handling:
...
3. **Terminology status:** `terminology-status: current|deprecated|emerging|archival`

### Definition Linking Strategies

- **Automatic link generation** for recognized domain terms
- **Plain-language fallback** with "Plain Language Summary" sections
- **Bidirectional terminology indexes** for cross-domain lookup (especially bilingual vaults)

### Open Questions

- Should terminology notes be universal (one definition per term) or domain-specific (different definitions for different contexts)?
- How do you handle terms that mean different things in different domains?
- Should search index include both jargon and common-language variants?
- When writing for universal audience, what's the jargon threshold?
