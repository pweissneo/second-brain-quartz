...
### Principle vs Parameters (Seed refinement)

Notes containing both timeless principles and time-sensitive parameters should separate them or note which is which. The principle may be evergreen, but the parameters (numbers, thresholds, current values) may need updating.

**Example:** A note about the 4% withdrawal rule should distinguish between:
- **Principle (evergreen):** "A sustainable withdrawal rate based on historical portfolio performance"
- **Parameter (time-sensitive):** "4% as a safe initial withdrawal rate" → should have `as-of: 2024` and be reviewed periodically

**Test:** For numeric guidance: (1) Is the number tied to current conditions? (2) Should it have `last-reviewed` and `as-of:` dates?
...
- [[Seed Refinement - Enhanced Note Lifecycle Model]] - Deprecated (content integrated here 2026-04-07)
